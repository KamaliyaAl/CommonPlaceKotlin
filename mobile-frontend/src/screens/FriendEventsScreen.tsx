...
export default function FriendEventsScreen() {
  ...
  const friendId: string | undefined = route?.params?.userId;
  const initialName: string | undefined = route?.params?.userName;
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const registrations = await api.getRegistrations(friendId);
  ...
  const title = friendName ? `${friendName}'s events` : "Joined events";
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View style={s.emptyInner}>
              <MaterialCommunityIcons name="calendar-blank-outline" size={52} color="#ccc" />
              <Text style={s.emptyText}>No joined events</Text>
            </View>
          }
          renderItem={({ item }) => {
              ...
              >
                ...
                <View style={s.cardBody}>
                  ...
                  <View style={s.cardMeta}>
                    ...
                    <Text style={s.cardMetaText}>{formatCyprusDate(item.startTime ?? item.date)}</Text>
                    {item.price ? (
                      <>
...
const s = StyleSheet.create({
  ...
  title: { fontSize: 26, fontWeight: "800", color: "#111", marginTop: 16, marginHorizontal: 16, marginBottom: 12 },
  ...
  cardPast: { backgroundColor: "#F7F7F7", borderLeftColor: "#ccc" },
  thumb: { width: 68, height: 68, borderRadius: 12 },
  thumbPlaceholder: { backgroundColor: "#e0e0e0", alignItems: "center", justifyContent: "center" },
  cardBody: { flex: 1 },
  ...
  badgePast: { backgroundColor: "#EEEEEE" },
